...
async def test_chapter_store_initialization():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        store = ChapterStore(book_id=1, base_dir=temp_dir)
...
async def test_delete_chapter():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # 创建章节
        await store.create_chapter(chapter_id, content)
        assert await store.read_chapter(chapter_id) == content

        # 删除章节
        await store.delete_chapter(chapter_id)
        result = await store.read_chapter(chapter_id)
        assert result == ""  # 删除后应该返回空字符串
...
async def test_read_nonexistent_chapter_returns_empty():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # 读取不存在的章节应该返回空字符串
        result = await store.read_chapter(999)
        assert result == ""
...
async def test_multiple_books_isolation():
    """测试不同书籍之间的数据隔离"""
    with tempfile.TemporaryDirectory() as temp_dir:
        store1 = ChapterStore(book_id=1, base_dir=temp_dir)
        store2 = ChapterStore(book_id=2, base_dir=temp_dir)
...
async def test_compact_removes_deleted_chapters():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # 验证章节确实被标记为删除
        assert await store.read_chapter(2) == ""
...
async def test_index_persistence():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # 创建第二个实例,使用相同的目录和书ID
        store2 = ChapterStore(book_id=1, base_dir=temp_dir)
