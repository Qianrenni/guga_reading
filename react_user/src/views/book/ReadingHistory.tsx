...
import { useReadingHistoryStore, useBookShelfStore } from '@/store';
...
export default function ReadingHistory() {
  ...
  const shelfIds = useMemo(
    () => shelfStore.bookShelf.map((item) => item.id),
    [shelfStore.bookShelf],
  );
  ...
  return (
    <div>
      ...
      >
        {historyStore.getReadingHistory().map((item) => (
          <div
            key={item.id}
            className="bg-card"
            style={{
          ...
          >
            ...
            {showClose && !isLessThan768 && (
              <div
                ...
                onClick={() => historyStore.delete(item.id)}
            ...
            >
              ...
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                ...
                {!shelfIds.includes(item.id) && (
                  <Button
                    size="small"
                    onClick={() => shelfStore.add(item.id)}
                  >
                    加入书架
                  </Button>
                )}
