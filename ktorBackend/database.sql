...
(
    ...
    `resourceType`  ENUM ('BOOK', 'USER', 'PERMISSION', 'CHAPTER', 'SHELF') NOT NULL,
    `action`        ENUM ('READ', 'CREATE', 'UPDATE', 'DELETE', 'AUDIT', 'MANAGE') NOT NULL,
    ...
    UNIQUE KEY `index_unique_code` (`resourceType`, `action`, `scope`)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_0900_ai_ci;
...
(
    ...
    `code`        ENUM ('USER', 'REVIEWER', 'AUTHOR', 'ADMIN', 'SUPER_ADMIN') NOT NULL,
...
(
    `userId`    INT      NOT NULL,
    `roleId`    INT      NOT NULL,
    `grantedBy` INT          NULL DEFAULT NULL,
...
(
    ...
    `uniqueReaderCount`  INT           NOT NULL DEFAULT 0,
    `pageViewCount`      INT           NOT NULL DEFAULT 0,
    `totalDuration`      DECIMAL(10,2) NOT NULL DEFAULT 0.00,
    PRIMARY KEY (`id`),
    UNIQUE INDEX `uq_chapter_hour` (`bookId`, `chapterId`, `hourStart`),
...
(
    ...
    `eventType` ENUM ('ENTER', 'EXIT', 'HEARTBEAT') NOT NULL,
    PRIMARY KEY (`id`),
    INDEX `ix_event_time` (`eventTime`),
    INDEX `ix_user_book_chapter` (`userId`, `bookId`, `chapterId`, `eventTime`),
    INDEX `ix_book_chapter_time` (`bookId`, `chapterId`, `eventTime`),
